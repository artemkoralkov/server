...
def lesson_to_dict(lesson: str) -> 'dict[str, str] | dict[str, str | list[str]] | dict[str, None]':
    ...
    if lesson_split_by_spaces[0] == 'Физическая':
        ...
        return {'lesson_name': ''.join(lesson_split_by_enters[0]), 'lesson_teacher': teachers}
    if '\n' in lesson:
    ...
    if '+' in lesson:
        ...
        return {'lesson_name': lesson_name.strip(), 'lesson_teacher': lesson_teacher.strip(),
                'lesson_type': lesson_type.strip()}
    if '(' not in lesson:
        if '\n' not in lesson:
            space_indexes: list[int] = [m.start()
                                        for m in re.finditer(' ', lesson)]
            return {'lesson_name': lesson[:space_indexes[-3] + 1],
                    'lesson_teacher': lesson[space_indexes[-3] + 1:]}
        else:
            split_lesson = lesson.split(' ')
            return {'lesson_name': split_lesson[0].strip(
...
def excel_to_json(filename):
    ...
    for i in range(4, n, 3):
        ...
        for j in range(4, 74):
            tmp = i - 1
            tmp_lesson: dict[str, str] | dict[str, str |
                                              list[str]] | dict[str, None] = dict()
